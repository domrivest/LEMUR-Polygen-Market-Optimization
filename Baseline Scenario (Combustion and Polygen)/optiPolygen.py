""" Main file of the biomass system optimisation
Author : Dominic Rivest
Implementation of the MILP problem
System maintenance is taken into account (july break), MeOH peak burner activated, multiprocessing of the scenarios
Created in 2022 """
...
prices = {'Biomass' : 0.0171, 'Elec': 0.17, 'MeOH': 0.1085, 'H2': 0.2401, 'Heat': 0} # $/kWh 100 $/tonneAnhydre
power = np.multiply([1600, 3200, 4800, 6400, 8000], tau) # The nominal biomass input power of the system kWh/15 minutes
MeOHkWh2L = 1/(18.2/3.6) # HHV MeOH
MeOHkWh2LLHV = 1/(15.8/3.6) # HHV MeOH
...
MeOHBurnerEta = 0.822 # Efficiency of the meOh burner (0.95 LHV)
...
def polygenOpti(nominalPower, processNumber): # Polygen optimization function per nominal power
    # DF for results storage
    print('Le numéro du processus est', processNumber)
    ...
    for instance in range(0, int(np.floor(duration/1000)+1)):
        ...
        model.maxBiomassPower = pe.ConstraintList() # Max heating power Biomass
        for t in model.time:
    ...
    optimalValues.to_csv('CSV/optimalValues' + str(nominalPower) + '(' + str(int(nominalPower/tau)) + 'kW).csv')
    optimalValuesSum.to_csv('CSV/optimalValuesSum' + str(nominalPower) + '(' + str(int(nominalPower/tau)) + 'kW).csv')
    optimalValuesTotal.to_csv('CSV/optimalValuesTotal' + str(nominalPower) + '(' + str(int(nominalPower/tau)) + 'kW).csv')

if __name__ == '__main__':
    pool = multiprocessing.Pool() #use all available cores, otherwise specify the number you want as an argument
    for i in range(len(power)):
        pool.apply_async(polygenOpti, args=(power[i], i))
    pool.close()
    pool.join()
