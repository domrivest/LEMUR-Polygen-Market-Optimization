""" Optimisation for sensitvity analysis with multiprocessing
...
seriesOpti = ['BiomassInc15', 'BiomassDec15', 'ElecInc15', 'ElecDec15', 'MeOHInc15', 'MeOHDec15', 'H2Inc15', 'H2Dec15'] # Sensitivity analysis per serie change (and related name)
pricesGen = [{'Biomass' : 0.0171*1.15, 'Elec': 0.17, 'MeOH': 0.1085, 'H2': 0.2401, 'Heat': 0},
{'Biomass' : 0.0171*0.85, 'Elec': 0.17, 'MeOH': 0.1085, 'H2': 0.2401, 'Heat': 0},
{'Biomass' : 0.0171, 'Elec': 0.17*1.15, 'MeOH': 0.1085, 'H2': 0.2401, 'Heat': 0},
{'Biomass' : 0.0171, 'Elec': 0.17*0.85, 'MeOH': 0.1085, 'H2': 0.2401, 'Heat': 0},
{'Biomass' : 0.0171, 'Elec': 0.17, 'MeOH': 0.1085*1.15, 'H2': 0.2401, 'Heat': 0},
{'Biomass' : 0.0171, 'Elec': 0.17, 'MeOH': 0.1085*0.85, 'H2': 0.2401, 'Heat': 0},
{'Biomass' : 0.0171, 'Elec': 0.17, 'MeOH': 0.1085, 'H2': 0.2401*1.15, 'Heat': 0},
{'Biomass' : 0.0171, 'Elec': 0.17, 'MeOH': 0.1085, 'H2': 0.2401*0.85, 'Heat': 0}] # $/kWh 100 $/tonneAnhydre
minPower = 0 # Multiplied by nominal power to determine the system minimal power

power = np.multiply([1600, 3200, 4800, 6400, 8000], tau) # The nominal biomass input power of the system kWh/15 minu
nominalPower = power[4] # Highest system power (8000 kW) for sensitivity analysis
...
def sensiOpti(serieName, processNumber):
    print('This is process number', processNumber)
    # For loop of change in price
    prices = pricesGen[seriesOpti.index(serieName)]
    ...
    optimalValues.to_csv('CSV/sensiOpti/optimalValues' + serieName + '(' + str(int(nominalPower/tau)) + 'kW).csv')
    optimalValuesSum.to_csv('CSV/sensiOpti/optimalValuesSum' + serieName + '(' + str(int(nominalPower/tau)) + 'kW).csv')
    optimalValuesTotal.to_csv('CSV/sensiOpti/optimalValuesTotal' + serieName + '(' + str(int(nominalPower/tau)) + 'kW).csv')

if __name__ == '__main__':
    pool = multiprocessing.Pool() #use all available cores, otherwise specify the number you want as an argument
    for i in range(len(seriesOpti)):
        pool.apply_async(sensiOpti, args=(seriesOpti[i], i))
    pool.close()
    pool.join()